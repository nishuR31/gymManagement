...
export function Input({
  ...
  if (styleMode === 'clay') {
    containerClass += isFocused
      ...
      : `rounded-2xl border-2 border-transparent shadow-sm`;
    containerStyle.backgroundColor = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.04)';
  } else if (styleMode === 'glass' || styleMode === 'liquid-glass') {
    containerClass += isFocused
      ? `rounded-xl border border-primary`
      : `rounded-xl border border-white/10`;
    containerStyle.backgroundColor = isDark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.4)';
  ...
  return (
    <View className="mb-4">
      ...
      <View className={`${containerClass} ${className || ''}`} style={containerStyle}>
        {leftIcon ? (
