...
export function ProtectedRoute() {
  ...
  if (status === "idle" || status === "loading") {
    return (
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-backdrop px-6 text-foreground">
        ...
        <div className="relative grid justify-items-center gap-5 text-center animate-fade-in">
          <div className="relative grid h-20 w-20 place-items-center rounded-full border border-brand/40 bg-panel/85 shadow-sm">
            <LoaderCircle className="absolute h-20 w-20 animate-spin text-brand/45" strokeWidth={1.5} aria-hidden="true" />
            <Dumbbell className="h-8 w-8 text-primary-foreground" aria-hidden="true" />
          </div>
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-primary-foreground">ValorFitness</p>
