  ...
  return (
    <section className="grid max-w-7xl gap-6 animate-fade-in">
      <div className="panel-gradient flex flex-wrap items-end justify-between gap-3 rounded-lg border border-line p-4 shadow-soft">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-brand">Business Intelligence</p>
          <h2 className="mt-2 text-3xl font-black text-ink">Reports</h2>
          <p className="mt-1 text-sm text-ink-muted">Operational exports and cached aggregates</p>
        </div>
          ...
          {reportOptions.map((option) => (
            ...
            <button
              ...
              className={`inline-flex h-10 items-center gap-2 rounded-md px-3 text-sm font-bold transition focus-visible:focus-ring ${
                type === option.type ? "bg-brand text-panel" : "border border-line bg-panel text-ink-muted hover:bg-line-faint"
              }`}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {type === "revenue" ? (
            <label className="grid gap-2 text-sm font-medium text-ink">
              <span>Range</span>
              <select className="h-11 rounded-md border border-line bg-surface/70 px-3 outline-none transition hover:border-brand/50 focus:border-brand focus:ring-2 focus:ring-brand/25" value={range} onChange={(event) => setRange(event.target.value as PaymentAnalyticsRange)}>
            ...
            <div className="grid gap-3">
              {Object.entries(report.totals).map(([key, value]) => (
                <div key={key} className="rounded-md border border-line bg-surface p-3">
                  <p className="text-xs font-semibold uppercase text-ink-faint">{readableStatus(key)}</p>
                  <p className="numeric mt-1 text-2xl font-black text-ink">{key.toLowerCase().includes("cents") ? formatCents(value) : value}</p>
                </div>
              ))}
              <p className="text-xs font-semibold text-ink-faint">Generated {formatDateTime(report.generatedAt)}</p>
            </div>
          ) : null}
        </Card>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={report?.buckets ?? []}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "var(--color-ink-muted)" }} axisLine={{ stroke: "var(--color-line)" }} tickLine={{ stroke: "var(--color-line)" }} width={80} />
                <Tooltip
                ...
                />
                <Bar dataKey={report?.buckets.some((bucket) => bucket.amountCents !== undefined) ? "amountCents" : "count"} fill="var(--color-brand)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-line">
          {rows.map((row, index) => (
            <tr key={index}>
              {keys.map((key) => (
                <td key={key} className={`px-3 py-3 text-ink-muted ${typeof row[key] === "number" || key.toLowerCase().includes("id") || key.toLowerCase().includes("cents") ? "numeric" : ""}`}>
