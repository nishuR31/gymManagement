  ...
  return (
    <section className="grid max-w-7xl gap-6 animate-fade-in">
      <div className="panel-gradient flex flex-wrap items-end justify-between gap-3 rounded-lg border border-line p-4 shadow-soft">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-brand">Training Library</p>
          <h2 className="mt-2 text-3xl font-black text-ink">Workout & Diet Plans</h2>
          <p className="mt-1 text-sm text-ink-muted">Template library and member assignments</p>
        </div>
      ...
      <div className="flex w-fit rounded-md border border-line bg-panel p-1">
        {(["workout", "diet"] as const).map((item) => (
          <button key={item} className={`h-9 rounded px-3 text-sm font-bold capitalize transition focus-visible:focus-ring ${kind === item ? "bg-brand text-panel" : "text-ink-muted hover:text-ink"}`} onClick={() => setKind(item)}>
            {item}
          </button>
        ))}
      </div>
      ...
      <Card title={kind === "workout" ? "Workout Templates" : "Diet Templates"}>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {activeTemplates.map((template) => (
            <div key={template.id} className="group rounded-lg border border-line bg-surface p-4 shadow-soft transition hover:-translate-y-1 hover:border-brand">
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  <p className="font-black text-ink">{template.name}</p>
                  <p className="mt-1 text-xs font-semibold uppercase text-ink-faint">Template</p>
                </div>
                <div className="grid h-11 w-11 place-items-center rounded-md bg-line-faint text-brand transition group-hover:scale-105">
              ...
              <div className="mb-3 flex flex-wrap gap-2">
                ...
                <Button variant="secondary" className="h-9 px-3 text-accent" onClick={() => void deleteTemplate(kind, template)}>
                  <Trash2 className="h-4 w-4" aria-hidden="true" />
                  Delete
                </Button>
              </div>
              <div className="mb-3 flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 rounded bg-line-faint px-2 py-1 text-xs font-bold text-ink-muted">
                  <ClipboardList className="h-3.5 w-3.5 text-brand" aria-hidden="true" />
        ...
        {kind === "workout" ? (
          <div className="grid gap-2">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-bold text-ink">Exercises</p>
            ...
            {exercises.map((exercise, index) => (
              <div key={index} className="grid gap-2 rounded-md border border-line bg-surface p-2 sm:grid-cols-[minmax(0,1fr)_80px_80px_36px]">
                ...
                <Button variant="secondary" className="mt-7 h-9 w-9 px-0 text-accent" onClick={() => setExercises((current) => current.filter((_, itemIndex) => itemIndex !== index))}>
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-bold text-ink">Meals</p>
            ...
            {meals.map((meal, index) => (
              <div key={index} className="grid gap-2 rounded-md border border-line bg-surface p-2 sm:grid-cols-[minmax(0,1fr)_80px_80px_80px_80px_36px]">
                ...
                <Button variant="secondary" className="mt-7 h-9 w-9 px-0 text-accent" onClick={() => setMeals((current) => current.filter((_, itemIndex) => itemIndex !== index))}>
                  <X className="h-4 w-4" aria-hidden="true" />
                </Button>
              </div>
            ))}
      ...
      <div className="grid gap-3">
        <label className="grid min-w-0 gap-2 text-sm font-medium text-ink">
          ...
          <div className="relative">
            ...
            <input
              className="h-11 w-full rounded-md border border-line bg-surface/70 pl-9 pr-3 text-sm text-ink outline-none transition hover:border-brand/50 focus:border-brand focus:ring-2 focus:ring-brand/25"
        ...
        {memberResults.length > 0 ? (
          <div className="max-h-60 overflow-y-auto rounded-md border border-line bg-surface shadow-soft">
            {memberResults.map((member) => (
              <button
                key={member.id}
                type="button"
                className="flex w-full items-center gap-3 border-b border-line px-3 py-2 text-left transition last:border-b-0 hover:bg-line-faint focus-visible:focus-ring"
                onClick={() => {
              ...
              >
                <UserRound className="h-4 w-4 shrink-0 text-brand" aria-hidden="true" />
        ...
        <label className="grid gap-2 text-sm font-medium text-ink">
          <span>Template</span>
          <select className="h-11 rounded-md border border-line bg-surface/70 px-3 outline-none transition hover:border-brand/50 focus:border-brand focus:ring-2 focus:ring-brand/25" value={templateId} onChange={(event) => setTemplateId(event.target.value)}>
        ...
        {canPickTrainer ? (
          <label className="grid gap-2 text-sm font-medium text-ink">
            ...
            <select className="h-11 rounded-md border border-line bg-surface/70 px-3 outline-none transition hover:border-brand/50 focus:border-brand focus:ring-2 focus:ring-brand/25" value={trainerId} onChange={(event) => setTrainerId(event.target.value)}>
              <option value="">No trainer assigned</option>
              {trainerProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.userId}</option>)}
            </select>
          </label>
        ) : ownTrainerId ? (
          <p className="rounded-md bg-surface p-3 text-sm font-semibold text-ink-muted">Assigned as your trainer profile</p>
        ) : null}
