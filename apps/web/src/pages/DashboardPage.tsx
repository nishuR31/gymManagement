import type { AuditLogDto, DashboardSummaryDto, LowStockProductDto, MembershipSubscriptionDto, NotificationDto, PaymentDto, ProductDto, ProductOrderDto, MemberDto } from "@gym/shared";
...
export function DashboardPage() {
  ...
  return (
    <section className="grid max-w-7xl min-w-0 gap-6 animate-fade-in">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card title="Red List (Missing > 7 Days)">
          {isLoading ? <SkeletonRows /> : null}
          {!isLoading && (summary?.redListMembers?.length ?? 0) === 0 ? <EmptyState title="No members on the Red List" /> : null}
          <div className="grid gap-3">
            {summary?.redListMembers?.map((member) => <RedListRow key={member.id} member={member} />)}
          </div>
        </Card>
      </div>
    </section>
  ...
  return (
    <section className="grid max-w-7xl gap-6 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <Card className="hover:-translate-y-1 hover:border-primary">
          <PortalMetric icon={TrendingUp} title="Current Streak" value={currentMember?.streakDays ? `${currentMember.streakDays} Days` : '0 Days'} copy="Keep up the great work!" tone="success" />
        </Card>
      </div>
...
function RedListRow({ member }: { member: MemberDto }) {
  return (
    <div className="rounded-md border border-destructive bg-destructive/10 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="mt-1 flex gap-4 text-xs font-semibold text-muted-foreground">
            <span>{member.phone}</span>
            <span>{member.email}</span>
          </div>
        </div>
        <p className="text-xs font-bold text-destructive">
          {member.lastAttendanceDate ? formatRelativeTime(member.lastAttendanceDate) : 'Never attended'}
        </p>
      </div>
    </div>
  );
