...
import {
  ...
  NullInventoryAggregateCache,
  type InventoryAggregateCache
} from "./inventory-cache.service.js";
import { paymentAnalyticsCacheKey, type PaymentAnalyticsCache } from "./payment-cache.service.js";
...
export class InventoryService {
  public constructor(
    ...
    private readonly inventoryCache: InventoryAggregateCache = new NullInventoryAggregateCache(),
    private readonly paymentAnalyticsCache?: PaymentAnalyticsCache,
  ...
  public async listLowStock(actor: RequestActor): Promise<{ data: LowStockProductDto[] }> {
    ensureStaffOrAbove(actor.role);
    const cached = await this.inventoryCache.get<{ data: LowStockProductDto[] }>(inventoryLowStockCacheKey);
    if (cached) {
      return cached;
    }
    const data = (await this.inventoryRepository.listLowStock()).map(toProductDto);
    const result = { data };
    await this.inventoryCache.set(inventoryLowStockCacheKey, result);
    return result;
  }

  public async valuation(actor: RequestActor): Promise<InventoryValuationDto> {
    ensureAdminOrAbove(actor.role);
    const cached = await this.inventoryCache.get<InventoryValuationDto>(inventoryValuationCacheKey);
    if (cached) {
      return cached;
    }
    const result = await this.inventoryRepository.valuation();
    await this.inventoryCache.set(inventoryValuationCacheKey, result);
    return result;
  }
  ...
  private async invalidateInventoryAggregates(): Promise<void> {
    try {
      await this.inventoryCache.delete([inventoryLowStockCacheKey, inventoryValuationCacheKey]);
    } catch {
      return;
    }
  }
  ...
  private async invalidatePaymentAnalytics(): Promise<void> {
    if (!this.paymentAnalyticsCache) {
      return;
    }
    try {
      await this.paymentAnalyticsCache.delete(paymentRanges.map(paymentAnalyticsCacheKey));
    } catch {
      return;
    }
  }
