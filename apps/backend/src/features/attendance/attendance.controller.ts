...
export class AttendanceController {
  ...
  public checkIn = async (request: FastifyRequest, reply: FastifyReply): Promise<unknown> => {
    ...
    if (result.kind === "disambiguation") {
      sendSuccess(reply, "Success", 300, { matches: result.matches });
      return;
    }

    sendSuccess(reply, "Success", 201, { attendance: result.attendance });
  };
  ...
  public history = async (request: FastifyRequest): Promise<unknown> => {
    const actor = requireActor(request);
    const query = historyQuerySchema.parse(request.query);
    return this.attendanceService.getHistory(query.page, query.pageSize, actor);
  };
