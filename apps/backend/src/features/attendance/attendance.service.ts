...
import {
  ...
  NullAttendanceCacheService,
  type AttendanceCacheService
...
import type { MemberRecord, MemberRepository } from "../../features/member/member.repository.js";
...
export class AttendanceService {
  ...
  public async checkIn(input: CheckInInput, actor: RequestActor, context: RequestContext): Promise<CheckInResult> {
    ...
    try {
      ...
      await this.invalidateCacheServiceForAttendance(attendance);
  ...
  public async checkOut(input: CheckOutInput, actor: RequestActor, context: RequestContext): Promise<AttendanceDto> {
    ...
    await this.invalidateCacheServiceForAttendance(closed);
    await invalidateDashboardAndReports(this.cache);
  ...
  public async getHistory(page: number, pageSize: number, actor: RequestActor): Promise<PaginatedAttendanceDto> {
    ensureStaffOrAbove(actor.role);
    const { attendances, total } = await this.attendanceRepository.listHistory(page, pageSize);
    return {
      data: attendances.map(toAttendanceDto),
      pagination: {
        page,
        pageSize,
        total,
  ...
  public async autoCloseStaleAttendances(): Promise<AutoCheckoutResult> {
    ...
    for (const attendance of staleAttendances) {
      ...
      await this.invalidateCacheServiceForAttendance(closedAttendance);
      await invalidateDashboardAndReports(this.cache);
  ...
  public async getDailyAttendance(date: string, actor: RequestActor): Promise<DailyAttendanceDto> {
    ...
    const cached = await this.cache?.get<DailyAttendanceDto>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  public async getMonthlyAttendance(month: string, actor: RequestActor): Promise<MonthlyAttendanceDto> {
    ...
    const cached = await this.cache?.get<MonthlyAttendanceDto>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  private async invalidateCacheServiceForAttendance(attendance: AttendanceRecord): Promise<void> {
    const date = toDateKey(attendance.checkInAt);
    const month = date.slice(0, 7);

    try {
      await this.cache?.delete([attendanceDailyCacheKey(date), attendanceMonthlyCacheKey(month)]);
    } catch {
      return;
    }
  }
