...
export async function staffRoutes(app: FastifyInstance, options: StaffRoutesOptions): Promise<void> {
  ...
  server.get("/profiles", async (request) => options.staffService.listProfiles(requireActor(request)));
  ...
  server.post("/profiles", async (request, reply) => {
    ...
    sendSuccess(reply, "Success", 201, { profile });
  });

  server.patch("/profiles/:id", async (request) => {
    const actor = requireActor(request);
  ...
  server.post("/profiles/:id/login", async (request, reply) => {
    const actor = requireActor(request);
    const params = idParamsSchema.parse(request.params);
  ...
  server.get("/leave-requests", async (request) => {
    const actor = requireActor(request);
  ...
  server.post("/leave-requests", async (request, reply) => {
    const actor = requireActor(request);
    ...
    sendSuccess(reply, "Success", 201, { leaveRequest });
  });

  server.post("/leave-requests/:id/review", async (request) => {
    const actor = requireActor(request);
    const params = idParamsSchema.parse(request.params);
