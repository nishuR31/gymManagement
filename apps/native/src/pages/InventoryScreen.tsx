          ...
          <CardContent className="p-0">
            {filteredProducts.length === 0 && !isLoading ? (
              <View className="items-center py-12 px-4">
                <Box size={32} color={activeColors.mutedForeground} />
                <Text className="font-bold text-foreground mt-2">No products found</Text>
              </View>
            ) : (
              <View>
                    ...
                    <View key={product.id} className={`flex-row justify-between items-center p-4 ${index !== filteredProducts.length - 1 ? 'border-b border-border' : ''}`}>
                      <View className="flex-row items-center flex-1 mr-2">
                        ...
                        <View className="flex-1">
                          ...
                          <Text className="text-xs font-semibold text-muted-foreground">{product.category} · {formatCents(product.priceCents)}</Text>
                        </View>
                      </View>
                      <View className="items-end">
                        <Text className={`font-black text-lg ${isLowStock ? 'text-destructive' : 'text-foreground'}`}>
