          ...
          <CardContent className="p-0">
            {filteredOrders.length === 0 && !isLoading ? (
              <View className="items-center py-12 px-4">
                <ClipboardList size={32} color={activeColors.mutedForeground} />
                <Text className="font-bold text-foreground mt-2">No orders found</Text>
              </View>
            ) : (
              <View>
                  ...
                  >
                    <View className="flex-row items-center flex-1 mr-2">
                      ...
                      <View className="flex-1">
                        ...
                        <Text className="text-xs font-semibold text-muted-foreground">{formatDateTime(order.createdAt)} · {order.productName}</Text>
                      </View>
                    </View>
                    <View className="items-end">
                      <Text className="font-black text-foreground text-lg">{formatCents(order.amountCents)}</Text>
              ...
              <ScrollView className="p-4" contentContainerStyle={{ paddingBottom: 60 }}>
                <Card className="mb-4">
                  <CardContent className="p-4 gap-4">
                    <View>
                      <Text className="text-xs font-bold text-muted-foreground uppercase">Order Code</Text>
                      <Text className="text-foreground mt-1">{selectedOrder.orderCode}</Text>
                    </View>
                    <View className="h-px bg-border" />
                    <View>
                      <Text className="text-xs font-bold text-muted-foreground uppercase">Product</Text>
                      <Text className="text-foreground mt-1">{selectedOrder.productName}</Text>
                    </View>
                    <View className="h-px bg-border" />
                    <View>
                      <Text className="text-xs font-bold text-muted-foreground uppercase">Customer</Text>
                      <Text className="text-foreground mt-1">{selectedOrder.memberName} · {selectedOrder.memberPhone}</Text>
                    </View>
                    <View className="h-px bg-border" />
                    <View>
                      <Text className="text-xs font-bold text-muted-foreground uppercase">Total</Text>
                      <Text className="text-foreground mt-1 font-semibold">{formatCents(selectedOrder.amountCents)}</Text>
                    </View>
                  </CardContent>
                </Card>
              </ScrollView>
